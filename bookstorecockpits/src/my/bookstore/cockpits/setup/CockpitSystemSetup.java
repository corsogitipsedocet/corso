 ...
 */
package my.bookstore.cockpits.setup;
...
import my.bookstore.cockpits.constants.BookstoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = BookstoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(BookstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(BookstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						BookstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/bookstorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
